...
class AnalyticsEngine:
    ...
    # ------------------------------------------------------- built-in intents
    # ------------------------------------------------------- intent menu
    # Named menu of everything the engine can answer. Used two ways:
    #   1. shown to the user when nothing matches (self-documenting), and
    #   2. given to the local LLM as a MULTIPLE-CHOICE question when keyword
    #      matching misses. Picking from a labeled menu is reliable even for a
    #      small model — far more reliable than writing SQL freehand — so
    #      arbitrary phrasings ("consolidate a report for spend analytics")
    #      land on the right template without hardcoding every wording.
    INTENT_MENU = {
        "consolidated_report": "full consolidated analysis report across all events (tables + insights)",
        "spend_report": "spend report: totals, vendors, monthly pattern, lanes",
        "spend_by_vendor": "total/average spend grouped by vendor/transporter",
        "vendor_concentration": "each vendor's % share of total spend (dependency risk)",
        "monthly_pattern": "spend and events grouped by month (buying pattern over time)",
        "lane_analysis": "spend/frequency by route corridor (origin-destination)",
        "cost_per_kg_ranking": "cheapest or most expensive lanes by cost per kg",
        "price_history": "rates over time (historical pricing trend)",
        "supplier_performance": "per-vendor wins, awarded totals, best/avg cost per kg",
        "counts": "how many events/files/line items are indexed",
        "event_lookup": "details of one or more specific EVN events",
    }

    def _sql_for_named_intent(self, name: str, q: str) -> Optional[str]:
        """Map a menu name to its SQL (reusing the same templates)."""
        proxies = {
            "spend_by_vendor": "total spend by vendor",
            "vendor_concentration": "vendor spend concentration",
            "monthly_pattern": "spend by month",
            "lane_analysis": "lane analysis top routes",
            "cost_per_kg_ranking": "cheapest cost per kg",
            "price_history": "historical price trend",
            "supplier_performance": "supplier insights summary",
            "counts": "how many events and files",
            "event_lookup": q,  # keeps the EVN ids from the original question
        }
        if name in ("consolidated_report", "spend_report"):
            return name  # handled specially by ask()
        proxy = proxies.get(name)
        return self._intent_sql(proxy) if proxy else None

    def _menu_pick(self, q: str) -> Optional[str]:
        """One cheap multiple-choice LLM call: which named intent fits?"""
        if not self.llm_fn:
            return None
        menu = "\n".join(f"- {k}: {v}" for k, v in self.INTENT_MENU.items())
        out = (self.llm_fn(
            "Pick which ONE named query best answers the user's question. "
            "Reply with ONLY the name, exactly as written, or CUSTOM if none fit.\n\n"
            f"Named queries:\n{menu}\n\nUser question: {q}\nName:") or "").strip()
        # tolerate small-model noise: take the first token, strip punctuation
        name = re.split(r"[\s:,.]", out)[0].strip().lower()
        return name if name in self.INTENT_MENU else None
    ...
    def ask(self, question: str) -> Dict[str, Any]:
        # Layer 1 — keyword templates (fast path, no LLM, always exact)
        sql = self._intent_sql(question)
        mode = "intent"
        # Layer 2 — LLM multiple-choice: which named template fits? Handles
        # arbitrary phrasings ("consolidate a report for spend analytics")
        # without hardcoding every wording; picking from a menu is reliable
        # for a small model in a way freehand SQL is not.
        if sql is None:
            picked = self._menu_pick(question)
            if picked in ("consolidated_report", "spend_report"):
                return {"mode": "menu", "report": picked}
            if picked:
                sql, mode = self._sql_for_named_intent(picked, question), "menu"
        # Layer 3 — freehand text-to-SQL (validated, retried once, refused if bad)
        if sql is None:
            sql, mode = self._llm_sql(question), "llm_sql"
        if sql is None:
            menu = "; ".join(v for v in self.INTENT_MENU.values())
            return {"mode": "unsupported", "error":
                    "Couldn't map this question to a query. I can answer: " + menu + "."}
        try:
            res = self._run(sql)
