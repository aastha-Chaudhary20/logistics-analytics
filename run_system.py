...
class ServiceManager:
    ...
    def _get_service_configs(self) -> Dict[str, ServiceConfig]:
        ...
        base_configs = {
            ...
            'frontend': ServiceConfig(
                name='frontend',
                # Detect Windows (os.name == 'nt') and use npm.cmd
                command=['npm.cmd' if os.name == 'nt' else 'npm', 'run', 'dev' if self.mode == 'dev' else 'start'],
                port=3000,
                startup_delay=5,
                required=False  
            )
        ...
        if self.mode == 'prod':
            ...
            base_configs['frontend'].command = [npm_cmd, 'run', 'start']
    ...
    def check_prerequisites(self) -> bool:
        ...
        npm_cmd = 'npm.cmd' if os.name == 'nt' else 'npm'
        if not self._command_exists(npm_cmd):
            self.logger.warning(f"{npm_cmd} not found - frontend will be disabled")
    ...
    def _command_exists(self, command: str) -> bool:
        ...
        try:
            ...
            subprocess.run([command, '--version'], 
                         capture_output=True, check=True, timeout=5, shell=use_shell)
            return True
